// Code generated by protoc-gen-go. DO NOT EDIT.
// versions:
// 	protoc-gen-go v1.35.1
// 	protoc        v3.12.4
// source: protobuf/profile.proto

package protobuf

import (
	protoreflect "google.golang.org/protobuf/reflect/protoreflect"
	protoimpl "google.golang.org/protobuf/runtime/protoimpl"
	reflect "reflect"
	sync "sync"
)

const (
	// Verify that this generated code is sufficiently up-to-date.
	_ = protoimpl.EnforceVersion(20 - protoimpl.MinVersion)
	// Verify that runtime/protoimpl is sufficiently up-to-date.
	_ = protoimpl.EnforceVersion(protoimpl.MaxVersion - 20)
)

type ProfileAddRequest struct {
	state         protoimpl.MessageState
	sizeCache     protoimpl.SizeCache
	unknownFields protoimpl.UnknownFields

	SessionId string `protobuf:"bytes,1,opt,name=sessionId,proto3" json:"sessionId,omitempty"`
}

func (x *ProfileAddRequest) Reset() {
	*x = ProfileAddRequest{}
	mi := &file_protobuf_profile_proto_msgTypes[0]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *ProfileAddRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*ProfileAddRequest) ProtoMessage() {}

func (x *ProfileAddRequest) ProtoReflect() protoreflect.Message {
	mi := &file_protobuf_profile_proto_msgTypes[0]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use ProfileAddRequest.ProtoReflect.Descriptor instead.
func (*ProfileAddRequest) Descriptor() ([]byte, []int) {
	return file_protobuf_profile_proto_rawDescGZIP(), []int{0}
}

func (x *ProfileAddRequest) GetSessionId() string {
	if x != nil {
		return x.SessionId
	}
	return ""
}

type ProfileAddResponse struct {
	state         protoimpl.MessageState
	sizeCache     protoimpl.SizeCache
	unknownFields protoimpl.UnknownFields

	SessionId string `protobuf:"bytes,1,opt,name=sessionId,proto3" json:"sessionId,omitempty"`
}

func (x *ProfileAddResponse) Reset() {
	*x = ProfileAddResponse{}
	mi := &file_protobuf_profile_proto_msgTypes[1]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *ProfileAddResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*ProfileAddResponse) ProtoMessage() {}

func (x *ProfileAddResponse) ProtoReflect() protoreflect.Message {
	mi := &file_protobuf_profile_proto_msgTypes[1]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use ProfileAddResponse.ProtoReflect.Descriptor instead.
func (*ProfileAddResponse) Descriptor() ([]byte, []int) {
	return file_protobuf_profile_proto_rawDescGZIP(), []int{1}
}

func (x *ProfileAddResponse) GetSessionId() string {
	if x != nil {
		return x.SessionId
	}
	return ""
}

var File_protobuf_profile_proto protoreflect.FileDescriptor

var file_protobuf_profile_proto_rawDesc = []byte{
	,
}

var (
	file_protobuf_profile_proto_rawDescOnce sync.Once
	file_protobuf_profile_proto_rawDescData = file_protobuf_profile_proto_rawDesc
)

func file_protobuf_profile_proto_rawDescGZIP() []byte {
	file_protobuf_profile_proto_rawDescOnce.Do(func() {
		file_protobuf_profile_proto_rawDescData = protoimpl.X.CompressGZIP(file_protobuf_profile_proto_rawDescData)
	})
	return file_protobuf_profile_proto_rawDescData
}

var file_protobuf_profile_proto_msgTypes = make([]protoimpl.MessageInfo, 2)
var file_protobuf_profile_proto_goTypes = []any{
	(*ProfileAddRequest)(nil),  // 0: protobuf.ProfileAddRequest
	(*ProfileAddResponse)(nil), // 1: protobuf.ProfileAddResponse
}
var file_protobuf_profile_proto_depIdxs = []int32{
	0, // 0: protobuf.Profile.Add:input_type -> protobuf.ProfileAddRequest
	1, // 1: protobuf.Profile.Add:output_type -> protobuf.ProfileAddResponse
	1, // [1:2] is the sub-list for method output_type
	0, // [0:1] is the sub-list for method input_type
	0, // [0:0] is the sub-list for extension type_name
	0, // [0:0] is the sub-list for extension extendee
	0, // [0:0] is the sub-list for field type_name
}

func init() { file_protobuf_profile_proto_init() }
func file_protobuf_profile_proto_init() {
	if File_protobuf_profile_proto != nil {
		return
	}
	type x struct{}
	out := protoimpl.TypeBuilder{
		File: protoimpl.DescBuilder{
			GoPackagePath: reflect.TypeOf(x{}).PkgPath(),
			RawDescriptor: file_protobuf_profile_proto_rawDesc,
			NumEnums:      0,
			NumMessages:   2,
			NumExtensions: 0,
			NumServices:   1,
		},
		GoTypes:           file_protobuf_profile_proto_goTypes,
		DependencyIndexes: file_protobuf_profile_proto_depIdxs,
		MessageInfos:      file_protobuf_profile_proto_msgTypes,
	}.Build()
	File_protobuf_profile_proto = out.File
	file_protobuf_profile_proto_rawDesc = nil
	file_protobuf_profile_proto_goTypes = nil
	file_protobuf_profile_proto_depIdxs = nil
}
